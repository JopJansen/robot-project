...
def main():
	...
	for _ in range(4):
		...
		print("== Wacht op sorteerposities op het topic '/sorteer_positie' ==")
		msg = rospy.wait_for_message('/sorteer_positie', String)

			# Abonneer op het topic '/sorteer_positie' en stuur de robot naar de ontvangen named target
		
    		positie = msg.data  # Haal de naam van de doelpositie op uit het bericht (bijv. 'bak_rb')
		print("== ga naar " + positie + " ==")  # Print naar welke positie de robot gaat

    		group.set_start_state_to_current_state()  # Gebruik de huidige robotpositie als startpunt
    		target_values = group.get_named_target_values(positie)  # Haal joint-waarden op voor deze named target
    		group.go(target_values, wait=True)  # Beweeg naar het doel en wacht tot de beweging klaar is

				
			

		# === Houd het script actief zodat het berichten blijft ontvangen ===
		#rospy.spin()
	...
	# === Naar 'resting' positie gaan ===
	print("== ga naar rustpositie ==")
